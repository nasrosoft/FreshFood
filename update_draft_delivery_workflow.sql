-- ===============================================================================
-- DEFINITIVE DRAFT ORDER & LIVRÉE COMMIT ARCHITECTURE
-- ===============================================================================

-- 1. DROP old conflicting triggers that modified stock or credit on intermediate edits
DROP TRIGGER IF EXISTS trigger_delivery_item_modified ON delivery_items;
DROP TRIGGER IF EXISTS trigger_process_delivery_stock ON delivery_orders;
DROP FUNCTION IF EXISTS handle_delivery_item_modification();
DROP FUNCTION IF EXISTS process_delivery_stock();

-- 2. Make customer credit trigger fully self-healing on INSERT, UPDATE, or DELETE
CREATE OR REPLACE FUNCTION update_customer_credit()
RETURNS TRIGGER AS $$
DECLARE
    v_cust_id UUID;
    v_total_debt DECIMAL(12,2) := 0.00;
...
BEGIN
    ...
    IF v_cust_id IS NOT NULL THEN
        SELECT 
            COALESCE(SUM(CASE WHEN transaction_type = 'DEBT' THEN amount ELSE 0.00 END), 0.00),
            COALESCE(SUM(CASE WHEN transaction_type = 'PAYMENT' THEN amount ELSE 0.00 END), 0.00)
        INTO v_total_debt, v_total_paid
        FROM credit_transactions
        WHERE customer_id = v_cust_id;
        
        UPDATE customers 
        SET current_credit = GREATEST(0.00, v_total_debt - v_total_paid) 
        WHERE id = v_cust_id;
    END IF;
...
DROP TRIGGER IF EXISTS trigger_update_customer_credit ON credit_transactions;
...
-- 3. Update process_sale function (Draft order creation)
...
-- 4. Update sync_delivery_completion function (LIVRÉE Commit Point)
